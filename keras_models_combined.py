...
class RecursivePrediction(Network):
    ...
    def _init_vars(self, config, settings=None, **kwargs):
        ...
        self.ae = None
        self.pred = None
        # 这部分在Autoencoder(Network)函数内也是重复的
        ...
        # 重复结束
        self.decode_predictions = kwargs.get("decode_predictions", False)#true
        ...
        self.pred_gradient_loss = kwargs.get("pred_gradient_loss", False)#true
        self.ls_prediction_loss = kwargs.get("ls_prediction_loss", False)
        self.ls_supervision = kwargs.get("ls_supervision", False)#true
        self.sqrd_diff_loss = kwargs.get("sqrd_diff_loss", False)
        self.ls_split = kwargs.get("ls_split", 0.0)#0.66
        ...
        if self.decode_predictions:#go
            ...
            self.loss_weights.append(1.0)
        if not self.train_prediction_only:#go
            #？可能是Lsup，c的监督损失
        ...
        if self.ls_prediction_loss:
            ...
            self.loss_weights.append(1.0)
        if self.ls_split > 0.0:#go
            #潜在空间细分损失
            self.ls_split_idx = int(self.z_num * self.ls_split)#16*0.66
    ...
    def _build_model(self, **kwargs):
        ...
        if batch_manager is None and self.advection_loss > 0.0:#advection_loss is 0.0
        ...
        enc = self.ae._encoder#encoder model
        dec = self.ae._decoder#decoder model
        ...
        #s[:-1] # 等价于 s[0:len(s)-1]，除了最后一个元素的切片
...
if __name__ == "__main__":
    ...
    decode_predictions = config.decode_predictions#True
    skip_pred_steps = config.skip_pred_steps
    init_state_network = config.init_state_network
    in_out_states = config.in_out_states
    pred_gradient_loss = config.pred_gradient_loss#True
    ls_prediction_loss = config.ls_prediction_loss
    ls_supervision = config.ls_supervision#True
    sqrd_diff_loss = config.sqrd_diff_loss
    ls_split = config.ls_split
    test_data_types = config.data_type.copy()#浅拷贝（copy），指向同一个对象，config.data_type变化会引起test_data_types相同的变化，与赋值类似，b=a。深拷贝（deepcopy）：分别指向独立的对象，a变化不会引起b变化
    #？把inflow移除了那不是dataset数据集里的i可以不要
    ...
    in_out_dim = in_out_dim + 1 if config.is_3d else in_out_dim#加上速度的z
